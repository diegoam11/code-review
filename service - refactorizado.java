...
public class GrupoHorarioServiceImpl implements GrupoHorarioService {
    ...
    // INICIO CÓDIGO NUEVO
    @Override
    public List<AulaEntity> AulasDisponibles(GrupoHorarioEntity grupoHorario, List<AulaEntity> aulas, List<GrupoHorarioEntity> horariosFilteredBySemestre) {
        List<AulaEntity> aulasDisponibles = new ArrayList<>();

        for (AulaEntity aula : aulas) {
            if (esAulaDisponible(aula, grupoHorario, horariosFilteredBySemestre)) {
    ...
    private boolean esAulaDisponible(AulaEntity aula, GrupoHorarioEntity grupoHorario, List<GrupoHorarioEntity> horariosFilteredBySemestre) {
        for (GrupoHorarioEntity grupo : horariosFilteredBySemestre) {
            if (hayConflictoDeHorario(grupo.getGruhorTHoraInicio().toLocalTime(), grupo.getGruhorTHoraFinal().toLocalTime(),
                                    grupoHorario.getGruhorTHoraInicio().toLocalTime(), grupoHorario.getGruhorTHoraFinal().toLocalTime())) {
                return false;
            }
        }
        return true;
